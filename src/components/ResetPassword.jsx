...
export default function ResetPassword() {
  ...
  async function handleSubmit(event) {
    ...
    setError("");
    if (!email) return setError("Missing email. Please go back and request a new code.");
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-slate-100 dark:bg-slate-900">
      <form onSubmit={handleSubmit} className="w-full max-w-md rounded-2xl bg-white p-8 shadow-lg dark:bg-slate-800">
        ...
        {message ? <p className="mt-5 rounded-lg bg-green-50 px-3 py-2 text-center text-sm text-green-700 dark:bg-green-900/20 dark:text-green-400">{message} <Link to="/login" className="font-semibold underline">Log in</Link></p> : <>
          <label className="mt-5 block text-sm font-medium">Verification code</label>
          <input type="text" value={code} onChange={(event) => setCode(event.target.value)} required minLength="6" maxLength="6" placeholder="123456" className="mt-1 w-full tracking-widest text-center rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-slate-600 dark:bg-slate-900" />
          <label className="mt-4 block text-sm font-medium">New password</label>
